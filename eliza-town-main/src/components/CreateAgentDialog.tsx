...
const DEFAULT_PLAN = STARTER_AGENT.plan;
...
export default function CreateAgentDialog({ isOpen, onClose, onCreateCharacter }: Props) {
  ...
  const selectableCharacters = hasCustomCharacters ? customCharacters : builtInCharacters;
  ...
  useEffect(() => {
    ...
    setName(STARTER_AGENT.name);
    setIdentity(STARTER_AGENT.identity);
    setPlan(STARTER_AGENT.plan);
    setPersonality([...STARTER_AGENT.personality]);
    if (selectableCharacters.length === 0) {
      setSelectedId(null);
      return;
    }
    const preferred =
      selectableCharacters.find((c) => c.name === STARTER_AGENT.character)?.name ??
      selectableCharacters[0].name;
    setSelectedId(preferred);
  }, [isOpen, selectableCharacters]);
      ...
      <div className="flex flex-col h-full bg-[#23202b] text-white font-dialog">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b-4 border-[#4a3b5b] bg-[#2d2438]">
          <div className="flex items-center gap-3">
             ...
             <p className="text-[10px] text-[#6d607d] mt-1 normal-case tracking-normal">
               Starter defaults are pre-filled — edit anything before creating.
             </p>
             </div>
          </div>
          <button
