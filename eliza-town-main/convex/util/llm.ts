...
import { OPENROUTER_DEFAULTS } from '../../data/openrouterDefaults';

const OPENAI_EMBEDDING_DIMENSION = 1536;
const OPENROUTER_EMBEDDING_DIMENSION = OPENROUTER_DEFAULTS.EMBEDDING_DIMENSIONS;
...
function resolveEmbeddingDimension(): number {
  // Schema evaluation cannot read env vars — keep this a compile-time constant.
  // Matches nvidia/llama-nemotron-embed-vl-1b-v2:free (free on OpenRouter).
  return OPENROUTER_EMBEDDING_DIMENSION;
}

export const EMBEDDING_DIMENSION: number = resolveEmbeddingDimension();
  ...
  switch (EMBEDDING_DIMENSION) {
    case OPENAI_EMBEDDING_DIMENSION:
      if (!process.env.OPENROUTER_API_KEY && !process.env.OPENAI_API_KEY) {
        throw new Error(
          "Add OPENROUTER_API_KEY to .env.local (recommended) or run: npx convex env set OPENROUTER_API_KEY 'your-key'",
        );
      }
      break;
...
export interface LLMConfig {
  provider: 'openrouter' | 'openai' | 'together' | 'ollama' | 'custom';
...
export function getLLMConfig(): LLMConfig {
  ...
  if (provider === 'openrouter' || (!provider && process.env.OPENROUTER_API_KEY)) {
    if (EMBEDDING_DIMENSION !== OPENROUTER_EMBEDDING_DIMENSION) {
      throw new Error(
        `EMBEDDING_DIMENSION must be ${OPENROUTER_EMBEDDING_DIMENSION} for OpenRouter free embeddings`,
      );
    }
    const baseUrl = process.env.OPENROUTER_BASE_URL ?? 'https://openrouter.ai/api/v1';
    return {
      provider: 'openrouter',
      url: baseUrl.replace(/\/v1\/?$/, '').replace(/\/$/, ''),
      chatModel: process.env.OPENROUTER_SMALL_MODEL ?? OPENROUTER_DEFAULTS.SMALL_MODEL,
      embeddingModel:
        process.env.OPENROUTER_EMBEDDING_MODEL ?? OPENROUTER_DEFAULTS.EMBEDDING_MODEL,
      stopWords: [],
...
const AuthHeaders = (): Record<string, string> => {
  ...
  if (config.provider === 'openrouter') {
    headers['HTTP-Referer'] = process.env.OPENROUTER_HTTP_REFERER ?? 'http://localhost:5173';
    headers['X-Title'] = process.env.OPENROUTER_APP_NAME ?? 'Eliza Town';
  }
  return headers;
};
  ...
  const {
    ...
    if (body.stream) {
      ...
      const message = json.choices[0]?.message as
        ...
        | undefined;
      const content = (message?.content ?? message?.reasoning ?? '').toString().trim();
      if (!content) {
        console.error('Unexpected result from OpenAI:', JSON.stringify(json).slice(0, 800));
        throw {
          retry: true,
          error: new Error('Chat completion returned empty content'),
        };
      }
      console.log(content);
      return content;
