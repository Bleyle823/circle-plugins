...
export function createPaymentRequest(params: {
  ...
  const uri =
    tokenAddress && chain.chainId != null
...
export function faucetInfo(chain: string): {
  chain: string;
  testnet: boolean;
  faucetUrl?: string;
  note: string;
...
export async function requestFaucet(params: {
  ...
  if (!info.testnet) {
    throw err(
      "MAINNET_BLOCKED",
      `${info.name} is a mainnet chain — the Circle faucet only dispenses testnet tokens. ` +
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (body?.message) message = `Faucet request failed: ${body.message}`;
    } catch {
  ...
  return {
    ...
    note:
      `Requested ${requestedList.join(" + ")} from the Circle faucet for ${params.address} ` +
