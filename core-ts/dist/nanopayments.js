...
async function loadX402(subpath = "") {
    const path = subpath
    ...
    try {
        ...
        throw err("DEPENDENCY_MISSING", `Nanopayments require the optional peer dependency ${path}. ` +
...
async function makeGatewayClient(config) {
    ...
    if (!GatewayClient) {
        throw err("DEPENDENCY_MISSING", "GatewayClient not found in @circle-fin/x402-batching/client.");
    }
...
export async function payX402(config, url, options) {
    ...
    try {
        const raw = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Nanopayment timed out after ${PAY_TIMEOUT_MS / 1000}s`)), PAY_TIMEOUT_MS)),
        ]);
        const payer = client.address;
        const gatewayTransferId = typeof raw?.transaction === "string" && isCircleUuid(raw.transaction)
            ? raw.transaction
            : undefined;
        return {
            url,
        ...
        if (msg.includes("timed out")) {
            throw err("UPSTREAM", `Nanopayment to ${url} timed out. The paywall must be running at http://localhost:4021/risk-profile.`);
        }
        if (msg.includes("status 404")) {
            throw err("UPSTREAM", `Nanopayment to ${url} returned 404. The endpoint must exist and respond with HTTP 402 Payment Required. ` +
        ...
        if (msg.includes("Invalid transfer ID") || msg.includes("params.id")) {
            throw err("UPSTREAM", "Gateway payment failed: invalid transfer ID. " +
...
export async function requirePayment(params) {
    ...
    if (!factory) {
        throw err("DEPENDENCY_MISSING", "createGatewayMiddleware not found in @circle-fin/x402-batching/server.");
    }
    return factory({
