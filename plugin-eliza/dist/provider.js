 ...
 */
export const circleWalletProvider = {
    name: "CIRCLE_WALLET",
    ...
    get: async (runtime, _message, _state) => {
        ...
        try {
            ...
            const walletId = runtime.getSetting?.("CIRCLE_WALLET_ID") ??
            ...
            if (walletId) {
                ...
                lines.push(`Agent wallet: ${walletId}${address ? ` (${address})` : ""}`, `USDC balance: ${usdc}`);
            ...
            if (faucet.faucetUrl)
                lines.push(`Faucet (testnet): ${faucet.faucetUrl}`);
            lines.push("", "Route chat requests to actions:", '- "balance" / "how much USDC" -> CIRCLE_CHECK_BALANCE', '- "send X USDC to 0x..." -> CIRCLE_SEND_USDC (confirm recipient + amount first)', '- "request/invoice X USDC" -> CIRCLE_REQUEST_USDC', '- "pay <url>" / nanopayment -> CIRCLE_PAY_X402', '- "deposit into gateway" -> CIRCLE_GATEWAY_DEPOSIT (needs confirm: true)', '- "gateway balance" -> CIRCLE_GATEWAY_BALANCE', "Mainnet and large transfers require confirm: true. Never ask the user for API keys or the entity secret.");
        }
        catch (e) {
            lines.push(`Circle wallet unavailable: ${e instanceof Error ? e.message : String(e)}`, "Set CIRCLE_API_KEY and ENTITY_SECRET to enable wallet actions.");
        }
