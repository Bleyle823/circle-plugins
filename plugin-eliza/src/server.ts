...
export async function startPaywalledServer(
  ...
  (gateway as any).onVerifyFailure(async (ctx: any) => {
  ...
  app.get("/risk-profile", (gateway as any).require("$0.01"), (_req, res) => {
    res.json({
      risk_score: 87,
      risk_level: "high",
  ...
  return new Promise((resolve) => {
    ...
    server.on("error", (error: NodeJS.ErrnoException) => {
      if (error.code === "EADDRINUSE") {
        logger.error(
          `[Circle] Port ${port} already in use — stale paywall may cause "Invalid transfer ID" errors. ` +
