...
export const circlePlugin = {
    name: "circle-plugins",
    description: "Circle + Arc agent wallet: balances, USDC transfers, x402 nanopayments, and payment requests.",
    ...
    init: async () => {
        ...
        const hasSecret = Boolean(process.env.ENTITY_SECRET?.trim() || process.env.CIRCLE_ENTITY_SECRET?.trim());
        if (!hasSecret)
            missing.push("ENTITY_SECRET");
        ...
        if (missing.length) {
            logger.warn(`[plugin-circle] Missing env: ${missing.join(", ")}. Wallet actions will fail until these are set.`);
        }
        if (!hasX402) {
            logger.warn(`[plugin-circle] Missing X402_PRIVATE_KEY / CLIENT_PRIVATE_KEY. Nanopayments will fail.`);
        }
        ...
        if (sellerAddress && serverKey) {
            try {
                ...
                const derived = privateKeyToAccount(serverKey).address;
                if (derived.toLowerCase() !== sellerAddress.toLowerCase()) {
                    logger.error(`[plugin-circle] SERVER_ADDRESS (${sellerAddress}) does not match SERVER_PRIVATE_KEY (${derived}). ` +
        ...
        if (sellerAddress && autoStart) {
            ...
            const kit = CircleAgentKit.create({
                ...
                entitySecret: process.env.ENTITY_SECRET ?? process.env.CIRCLE_ENTITY_SECRET ?? "dummy",
                x402PrivateKey: process.env.X402_PRIVATE_KEY ?? process.env.CLIENT_PRIVATE_KEY,
            ...
            logger.warn("[plugin-circle] SERVER_ADDRESS not set — x402 paywall server not started. " +
...
export { makeAction } from "./shared.js";
