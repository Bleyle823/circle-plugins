...
export const circleWalletProvider = {
    ...
    get: async (runtime, _message, _state) => {
        ...
        const hasX402 = Boolean(envSetting(runtime, "X402_PRIVATE_KEY") ?? envSetting(runtime, "CLIENT_PRIVATE_KEY"));
        const paywallUrl = envSetting(runtime, "X402_PAYWALL_URL") ?? defaultPaywallUrl();
        ...
        lines.push("", "When the user asks for live data, run the matching action (then REPLY with the result):", '- Gateway balance → CIRCLE_GATEWAY_BALANCE', '- Wallet USDC balance → CIRCLE_CHECK_BALANCE', '- Pay x402 paywall / risk profile → CIRCLE_PAY_X402', '- Deposit to Gateway → CIRCLE_GATEWAY_DEPOSIT', '- Send USDC → CIRCLE_SEND_USDC', '- Testnet faucet → CIRCLE_REQUEST_FAUCET', "", "For greetings and general chat, use REPLY only (no Circle action).");
        const values = {
