...
import { sendUsdcViaWalletsApi } from "./sendUsdc.js";
...
export const sendUsdcAction = makeAction({
  ...
  requiredParams: ["destinationAddress", "amount"],
  validate: async (_runtime, message) => {
    const text = String(message?.content?.text ?? "");
    return /\b(send|transfer|pay)\b/i.test(text) && /0x[a-fA-F0-9]{40}/.test(text);
  },
  run: async (kit, p) => {
    const walletId = (p.walletId as string) || process.env.CIRCLE_WALLET_ID!;
    const destinationAddress = p.destinationAddress as string;
    const amount = String(p.amount);
    try {
      return await kit.sendUSDC({
        walletId,
        destinationAddress,
        amount,
        chain: p.chain as string | undefined,
      ...
      // AppKit adapter may fail (Blockchain export mismatch). Fall back to wallets API.
      return sendUsdcViaWalletsApi({
        walletId,
        destinationAddress,
        amount,
        wait: p.wait !== false,
